...
<div>
    <div class="lw-section-heading-block">
        ...
        <div class="lw-section-right-content">
            <a href title="<?= __tr('Add New Supplier') ?>" ng-show="canAccess('manage.suppliers.write.create')" class="lw-btn btn btn-sm btn-primary float-right" ng-click="suppliersListCtrl.openAddDialog()">
            <i class="fa fa-plus"></i> <?= __tr('Add New Supplier') ?> </a>
        </div>
        <!--/ button -->
    ...
    <table class="table table-striped table-bordered" id="lwsuppliersList" class="ui celled table" cellspacing="0" width="100%">
        <thead>
            <tr>
                <th><?= __tr('Name') ?></th>
                <th width="20%"><?= __tr('Description') ?></th>
                <th><?= __tr('Action') ?></th>
            </tr>
        </thead>
        <tbody></tbody>
